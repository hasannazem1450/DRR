...
{
    public class ReadDoctorTreatmentCenterReservationQueryHandler : IQueryHandler<ReadDoctorTreatmentCenterReservationQuery, ReadDoctorReservationQueryResponse>
    {
        private IReservationRepository _reservatioRepository;
        private IReservationService _reservationService;
        public ReadDoctorTreatmentCenterReservationQueryHandler(IReservationRepository reservatioRepository, IReservationService reservationService)
        {
            _reservatioRepository = reservatioRepository;
            _reservationService = reservationService;
        }

        public async Task<ReadDoctorReservationQueryResponse> Execute(ReadDoctorTreatmentCenterReservationQuery query, CancellationToken cancellationToken)
        {
            var doctorReservation = await _reservatioRepository.ReadReservationByDoctorIdByTreatmentCenterId(query.DoctorId,query.TreatmentCenterId);
